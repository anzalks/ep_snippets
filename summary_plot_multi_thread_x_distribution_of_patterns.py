...
def pre_vs_post_avresp(points_or_pattern_file_set,title, fig, axs, plt_no):
    ...
    if title=='pattern':
        axs[plt_no].scatter(pre_x[0],pre[0], color='k')
        axs[plt_no].scatter(post_x[0],post[0], color='k', label='trained_pat')
...
def summation_plot(points_file,pattern_file,title,fig,axs,plt_no):
    ...
    if 'post' in title:
        ...
        axs[plt_no].scatter(sum_of_points[0],pattern_response[0], 
